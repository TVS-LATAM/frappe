export default class CardPreviewSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			frappe.throw(__("DocType required"));
		}

		console.log("CardPreviewSettings =< ", settings)
		...
		this.dialog = null;
		this.fields = this.settings ? this.settings.card_fields : [];
	...
	make() {
		this.dialog = new frappe.ui.Dialog({
			title: __("Card Preview Settings"),
			fields: [
				{
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			frappe.call({
				method: "frappe.desk.doctype.kanban_board.kanban_board.save_card_preview_settings",
				args: {
	...
	setup_fields() {
		...
		fields_html.html(`
			<div class="form-group">
				<div class="clearfix">
					<label class="control-label" style="padding-right: 0px;">${__("Card Preview Fields")}</label>
				</div>
				...
				<p class="help-box small text-muted">
					${__("Select fields to display in the kanban card preview")}
				</p>
			</div>
		`);
		...
		new Sortable(wrapper.getElementsByClassName("control-input-wrapper")[0], {
			...
			onUpdate: (params) => {
				...
				this.dialog.set_value("card_fields", JSON.stringify(this.fields));
				this.refresh();
			},
		});
	}
	...
	show_column_selector() {
		let dialog = new frappe.ui.Dialog({
			title: __("Select Card Preview Fields"),
			fields: [
				{
	...
	get_fields() {
		this.fields = this.settings.card_fields || [];
